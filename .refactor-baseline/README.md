...
- **N** — **modo demo (de la landing) y la tabla del historial**: en la **landing** la tarjeta avisa que la donación es una demo; el navbar tiene una sola ancla de panel, «Resumen» (N2b); donar en demo **no pide ninguna firma** (la wallet falsa del bloque sólo sabe identificarse: si la app intentara firmar, el flujo terminaría en error; N4), **no suma nada al resumen del panel** (N7–N8) y **no deja rastro local** (N8b); en demo el **dueño** también puede probar la donación en su propia página (N18). Y la **tabla del historial** muestra la historia **real**: el harness manda una donación de verdad (`fund(address)` desde una cuenta desbloqueada de anvil) a la cuenta conectada y verifica que la tabla liste las filas leídas de la chain (P5–P6), que venga autorizada a retirar (P7) y que al desconectar la vista del profesional se apague y las dos islas se desmonten (P8–P9). El bloque **R** carga la misma página con **`?demo=0`** para ejercitar el camino real (firma con el calldata del dueño): es el control positivo de que "no firmó" significa algo.
